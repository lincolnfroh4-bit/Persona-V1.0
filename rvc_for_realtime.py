...
try:
    ...
    FAISS_IMPORT_ERROR = ""
except Exception as faiss_import_exc:
    faiss = None
    FAISS_AVAILABLE = False
    FAISS_IMPORT_ERROR = str(faiss_import_exc)
...
class RVC:
    def __init__(
        ...
        try:
            global config
            self._index_runtime_disabled = False
            self.inp_q = inp_q
            self.opt_q = opt_q
            self.device = device
            self.f0_up_key = key
            ...
            if index_rate != 0:
                if not FAISS_AVAILABLE:
                    print(
                        "FAISS not available, so index search is disabled. "
                        f"Reason: {FAISS_IMPORT_ERROR}"
                    )
                    self.index_rate = 0
                elif not index_path or not os.path.exists(index_path):
                    print("Index path missing, so index search is disabled.")
                    self.index_rate = 0
                else:
    ...
    def infer(
        ...
        try:
            ...
            if not self._index_runtime_disabled:
                traceback.print_exc()
                print("index search failed at runtime; disabling index for stability.")
                self._index_runtime_disabled = True
            self.index_rate = 0
